...
parser.add_argument('-embedding',
                    default='./data/data/embedding.pt',
...
def initVocabulary(name, dataFile, vocabFile, vocabSize, char=False):
    ...
    if vocab is None:
        ...
        if name == 'target':
            genWordVocab = makeVocabulary(dataFile, vocabSize, True, char=char) 
        else:    
            genWordVocab = makeVocabulary(dataFile, vocabSize, False, char=char) 
        vocab = genWordVocab
...
def makeData(srcFile, tgtFile, srcDicts, tgtDicts, sort=False, char=False):
    ...
    print('Target dict size :', tgtDicts.size())
    return dataset(src, tgt, raw_src, raw_tgt, tgtDicts.size(),False) 
...
def get_word_embedding(words, model_path):
    ...
    emb = []
    for word in words:
        emb.append(model.get_word_vector(word))
    emb = np.vstack(emb)
...
def main():
    ...
    if opt.fasttext_model is not None:
        print('Preparing word embedding...')
        words_list = [ dicts['src'].getLabel(i) for i in range(dicts['src'].size())]
        print("Total number of source words :",len(words_list))
        embedding = get_word_embedding(words_list, opt.fasttext_model)
        torch.save(embedding, opt.embedding)
        print('Embedding shape : ', embedding.shape)
        print('Saving embedding to ', opt.embedding)
